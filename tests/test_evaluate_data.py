...
from process_data import process_full_mean_values, process_per_base_file, update_per_base_df
...
class TestTestPerBaseFile:
    ...
    def test_results_are_pass_fail_or_skip(self, processed_test_data):
        data, selected_means, full_means = processed_test_data
        result = run_test_per_base_file(data, selected_means, full_means)
        assert set(result["Result"].unique()).issubset({"Pass", "Fail", "Skip"})
    ...
    def test_all_selected_skips_ttest_no_crash(self, minimal_per_base_df):
        """When every row is selected, unselected group is empty — no exception."""
        processed = process_per_base_file(minimal_per_base_df, False)
        full_means = process_full_mean_values(processed)
        means = pd.DataFrame()  # unused by test_mean_values for groups
        result = run_test_mean_values(processed, means, full_means)
        assert not result.empty
        assert (result["Result"] == "Skip").all()
